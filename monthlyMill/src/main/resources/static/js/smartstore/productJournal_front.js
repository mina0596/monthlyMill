...
document.querySelector(".prevDateBtn").addEventListener("click", function(){
    ...
    const nowDate = new Date(tableDate.innerHTML);   
    const prevDate = new Date(nowDate.setDate(nowDate.getDate() - 1));

    tableDate.innerHTML = `${prevDate.getFullYear()}-${prevDate.getMonth()+1}-${("0"+prevDate.getDate()).slice(-2)}`;
});
//다음날로
document.querySelector(".nextDateBtn").addEventListener("click", function(){
    ...
    tableDate.innerHTML = `${nextDate.getFullYear()}-${nextDate.getMonth()+1}-${("0"+nextDate.getDate()).slice(-2)}`;
});
